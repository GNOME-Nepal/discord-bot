...
module.exports = {
  ...
  async execute(interaction) {
    if (interaction.replied || interaction.deferred)
      return console.error('Interaction already replied or deferred.');
    ...
    try {
      const { data: repoContributors } = await githubApi.get(
        '/repos/GNOME-Nepal/discord-bot/contributors',
      );
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          'Here is the current status of the bot and its top contributors:',
        )
        .addFields(
          ...
          { name: '📦 Discord.js Version', value: discordJsVersion },
          {
            ...
            value: `${repoContributorCount}`,
          },
        )
        .setFooter({
          text: `Data fetched from Hosting Environment & GitHub API | Top 3 Contributors: ${topContributors}`,
        });
